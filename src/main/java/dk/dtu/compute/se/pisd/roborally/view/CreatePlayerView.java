...
public class GameBoardView extends Application {
    ...
    private void renderBoard(BorderPane root, BoardDTO board, List<Player> players) {
        ...
        for (List<SpaceDTO> row : board.getSpaces()) {
            for (SpaceDTO space : row) {
                ...
                boardGrid.add(cell, space.getX(), board.getSpaces().size() - 1 - space.getY());
            }
        }
        ...
        for (Player player : players) {
            ...
            boardGrid.add(avatar, player.getX(), board.getSpaces().size() - 1 - player.getY());
        }
    ...
    private void updatePlayerPosition(Player player) {
        ...
        renderBoard((BorderPane) player.getScene().getRoot(), apiService.getBoardById(1), players);
    }
